...
(* Instructions to sink all ships in samplebag2. Helper for testing all_sunk. *)
  let sink_all = 
  let c = (SB.get_Carrier samplebag2) in let b = (SB.get_Battleship samplebag2) in 
  let d = (SB.get_Destroyer samplebag2) in let s = (SB.get_Submarine samplebag2) in 
  let pb = (SB.get_patrolBoat samplebag2) in 
  let set = [X.set_position c 0 0 0 4; X.set_position b 1 0 1 3; 
    X.set_position d 2 0 2 2; X.set_position s 3 0 3 2; X.set_position pb 4 0 4 1] 
  in 
  let sink = [X.hit_ship c (0, 0); X.hit_ship c (0, 1); X.hit_ship c (0, 2); 
    X.hit_ship c (0, 3); X.hit_ship c (0, 4); 
    X.hit_ship b (1, 0); X.hit_ship b (1, 1); X.hit_ship b (1, 2); 
    X.hit_ship b (1, 3); X.hit_ship d (2, 0); X.hit_ship d (2, 1); 
    X.hit_ship d (2, 2); 
    X.hit_ship s (3, 0); X.hit_ship s (3, 1); X.hit_ship s (3, 2);
    X.hit_ship pb (4, 0); X.hit_ship pb (4, 1);] in 
  ()
...
let ships_bag_tests = [
  ...
  ("bag all_sunk" >:: fun _ -> 
    assert_equal (false) (SB.all_sunk samplebag));
  ("bag all_sunk" >:: fun _ -> 
