...
module PlayerList : PlayerListOfShips = struct
  ...
  type t = {
    mutable carrier : AShip.t;
    mutable battleship : AShip.t;
    mutable destroyer : AShip.t;
    mutable submarine : AShip.t;
    mutable patrolBoat : AShip.t;
  } (** AF: Record with 5 fields that represents all 5 of the players ships RI:
     only 1 of each ship type in a player's bag *)
  ...
  let get_Carrier (bag : t) : AShip.t = bag.carrier
  let get_Battleship (bag : t) : AShip.t = bag.battleship
  let get_Destroyer (bag : t) : AShip.t = bag.destroyer
  let get_Submarine (bag : t) : AShip.t = bag.submarine
  let get_patrolBoat (bag : t) : AShip.t = bag.patrolBoat

  (* let remove_ship (player_ships : t) (sunk_ship : AShip.t) : t = let
     ship_type = AShip.get_type_of_ship sunk_ship in match ship_type with |
  ...
  let list_health (bag : t) : unit =
    ...
    print_endline
      ("Carrier Ship Health "
      ^ string_of_int (AShip.get_health bag.carrier)
      ^ " of 5");
    T.print_string [ T.Bold; Foreground Green ] "     + ";
    print_endline
      ("Battleship Ship Health "
      ^ string_of_int (AShip.get_health bag.battleship)
      ^ " of 4");
    T.print_string [ T.Bold; Foreground Green ] "     + ";
    print_endline
      ("Destroyer Ship Health "
      ^ string_of_int (AShip.get_health bag.destroyer)
      ^ " of 3");
    T.print_string [ T.Bold; Foreground Green ] "     + ";
    print_endline
      ("Submarine Ship Health "
      ^ string_of_int (AShip.get_health bag.submarine)
      ^ " of 3");
    T.print_string [ T.Bold; Foreground Green ] "     + ";
    print_endline
      ("Patrol Boat Ship Health "
      ^ string_of_int (AShip.get_health bag.patrolBoat)
      ^ " of 2");
    T.print_string [ T.Bold; Foreground Green ] "------------ \n"
  ...
  let all_sunk (bag : t) : bool =
    ...
    let rec check (funct_list : AShip.t list) : bool =
      ...
      | [] -> true
    in
    if check list_to_check then true else false
end
